...
final class PhabricatorRoleColumn
  extends PhabricatorRoleDAO
  implements
  ...
  public static function initializeNewColumn(PhabricatorUser $user) {
    return id(new PhabricatorRoleColumn())
      ->setName('')
  ...
  protected function getConfiguration() {
    return array(
      ...
      self::CONFIG_KEY_SCHEMA => array(
        'key_status' => array(
          'columns' => array('rolePHID', 'status', 'sequence'),
        ),
        'key_sequence' => array(
          'columns' => array('rolePHID', 'sequence'),
        ),
        'key_proxy' => array(
          'columns' => array('rolePHID', 'proxyPHID'),
          'unique' => true,
        ),
        'key_trigger' => array(
  ...
  public function generatePHID() {
    return PhabricatorPHID::generateNewPHID(
      PhabricatorRoleColumnPHIDType::TYPECONST);
  }

  public function attachRole(PhabricatorRole $role) {
    $this->role = $role;
    return $this;
  }

  public function getRole() {
    return $this->assertAttached($this->role);
  }
  ...
  public function getOrderingKey() {
    ...
    // Normal columns and subrole columns go first, in a user-controlled
    // order.
  ...
  public function attachTrigger(PhabricatorRoleTrigger $trigger = null) {
  ...
  public function canHaveTrigger() {
    // Backlog columns and proxy (subrole / milestone) columns can't have
  ...
  public function getWorkboardURI() {
    return $this->getRole()->getWorkboardURI();
  }
  ...
  public function getDropEffects() {
    ...
    $proxy = $this->getProxy();
    if ($proxy && $proxy->isMilestone()) {
      $effects[] = id(new PhabricatorRoleDropEffect())
...
/* -(  PhabricatorConduitResultInterface  )---------------------------------- */
  ...
  public function getFieldSpecificationsForConduit() {
    return array(
      ...
      id(new PhabricatorConduitSearchFieldSpecification())
        ...
        ->setDescription(pht('The role the column belongs to.')),
      id(new PhabricatorConduitSearchFieldSpecification())
        ->setKey('proxyPHID')
        ->setType('phid?')
        ->setDescription(
          pht(
            'For columns that proxy another object (like a subrole or '.
  ...
  public function getFieldValuesForConduit() {
    return array(
      ...
      'role' => $this->getRole()->getRefForConduit(),
    );
  }
...
/* -(  PhabricatorApplicationTransactionInterface  )------------------------- */
  ...
  public function getApplicationTransactionEditor() {
    return new PhabricatorRoleColumnTransactionEditor();
  }
...
/* -(  PhabricatorPolicyInterface  )----------------------------------------- */
  ...
  public function getPolicy($capability) {
    ...
    // them the same as the role's policies.
    switch ($capability) {
  ...
  public function hasAutomaticCapability($capability, PhabricatorUser $viewer) {
    return $this->getRole()->hasAutomaticCapability(
      $capability,
      $viewer);
  }

  public function describeAutomaticCapability($capability) {
    return pht('Users must be able to see a role to see its board.');
  }
