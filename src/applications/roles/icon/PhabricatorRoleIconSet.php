...
final class PhabricatorRoleIconSet
  extends PhabricatorIconSet {

  const ICONSETKEY = 'roles';

  const SPECIAL_MILESTONE = 'milestone';

  public function getSelectIconTitleText() {
    return pht('Choose Role Icon');
  }
  ...
  private static function getIconSpecifications() {
    return PhabricatorEnv::getEnvConfig('roles.icons');
  }
  ...
  public static function validateConfiguration($config) {
    if (!is_array($config)) {
      throw new Exception(
        pht('Configuration must be a list of role icon specifications.'));
    }
    ...
    foreach ($config as $idx => $value) {
      ...
      if (isset($keys[$key])) {
        throw new Exception(
          pht(
            'Role icons must have unique keys, but two icons share the '.
            'same key ("%s").',
            $key));
      } else {
        $keys[$key] = true;
      ...
      if ($image !== null) {
        ...
        $image = $root.'/resources/builtin/roles/'.$builtin;

        if (!array_key_exists($image, $builtin_map)) {
            throw new Exception(
              pht(
                'The role image ("%s") specified for ("%s") '.
                'was not found in the folder "resources/builtin/roles/".',
                $builtin,
                $key));
        }
      }
      ...
      if (idx($value, 'default')) {
        if ($default === null) {
          if ($is_disabled) {
            throw new Exception(
              pht(
                'The role icon marked as the default icon ("%s") must not '.
                'be disabled.',
                $key));
          }
          $default = $value;
      ...
      if ($special === self::SPECIAL_MILESTONE) {
        if ($milestone === null) {
          if ($is_disabled) {
            throw new Exception(
              pht(
                'The role icon ("%s") with special attribute "%s" must '.
                'not be disabled',
                $key,
    ...
    if ($default === null) {
      throw new Exception(
        pht(
          'Role icons must include one icon marked as the "%s" icon, '.
    ...
    if ($milestone === null) {
      throw new Exception(
        pht(
          'Role icons must include one icon marked with special attribute '.
  ...
  private static function getColorSpecifications() {
    return PhabricatorEnv::getEnvConfig('roles.colors');
  }
  ...
  public static function validateColorConfiguration($config) {
    if (!is_array($config)) {
      throw new Exception(
        pht('Configuration must be a list of role color specifications.'));
    }
    ...
    foreach ($config as $idx => $value) {
      ...
      if (isset($keys[$key])) {
        throw new Exception(
          pht(
            'Role colors must have unique keys, but two icons share the '.
            'same key ("%s").',
            $key));
      } else {
        $keys[$key] = true;
    ...
    if ($default === null) {
      throw new Exception(
        pht(
          'Role colors must include one color marked as the "%s" color, '.
