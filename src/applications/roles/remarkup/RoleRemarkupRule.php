...
final class RoleRemarkupRule extends PhabricatorObjectRemarkupRule {
  ...
  protected function getObjectIDPattern() {
    ...
    // Broadly, this will not match every possible role monogram, and we
    // accept some false negatives -- like `#dot.` -- in order to avoid a bunch
    // of false positives on general use of the `#` character.

    // In other contexts, the PhabricatorRoleRolePHIDType pattern is
  ...
  protected function loadObjects(array $ids) {
    ...
    $query->execute();
    $roles = $query->getNamedResults();

    // Slice the "#" off again.
    $result = array();
    foreach ($roles as $name => $role) {
      $result[substr($name, 1)] = $role;
    }
