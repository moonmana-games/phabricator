...
final class PhabricatorRoleImageTransaction
  extends PhabricatorRoleTransactionType {

  const TRANSACTIONTYPE = 'role:image';
  ...
  public function getTitle() {
    ...
    if (!$old) {
      return pht(
        "%s set this role's image to %s.",
        $this->renderAuthor(),
        $this->renderNewHandle());
    } else if (!$new) {
      return pht(
        "%s removed this role's image.",
        $this->renderAuthor());
    } else {
      return pht(
        "%s updated this role's image from %s to %s.",
        $this->renderAuthor(),
