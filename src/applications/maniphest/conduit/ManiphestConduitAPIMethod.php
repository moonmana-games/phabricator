...
abstract class ManiphestConduitAPIMethod extends ConduitAPIMethod {
  ...
  protected function buildTaskInfoDictionaries(array $tasks) {
    ...
    $all_deps = id(new PhabricatorEdgeQuery())
      ...
      ->withEdgeTypes(array(ManiphestTaskDependsOnTaskEdgeType::EDGECONST, ManiphestTaskBlockerEdgeType::EDGECONST));
    $all_deps->execute();
    ...
    foreach ($tasks as $task) {
      ...
      $task_deps = $all_deps->getDestinationPHIDs(
        array($task->getPHID()),
        array(ManiphestTaskDependsOnTaskEdgeType::EDGECONST, ManiphestTaskBlockerEdgeType::EDGECONST));
