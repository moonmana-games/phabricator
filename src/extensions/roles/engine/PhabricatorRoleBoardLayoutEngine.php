...
final class PhabricatorRoleBoardLayoutEngine extends Phobject {
  ...
  public function queueAddPosition(
    ...
    if (!$object_position) {
      $object_position = id(new PhabricatorRoleColumnPosition())
  ...
  public function applyPositionUpdates() {
    ...
    $table = new PhabricatorRoleColumnPosition();
  ...
  private function loadColumns(array $boards) {
    ...
    $columns = id(new PhabricatorRoleColumnQuery())
      ->setViewer($viewer)
      ->withRolePHIDs(array_keys($boards))
      ->needTriggers(true)
      ->execute();
    ...
    if ($need_children) {
      $children = id(new PhabricatorRoleQuery())
        ->setViewer($viewer)
        ->withParentRolePHIDs($need_children)
        ->execute();
      $children = mpull($children, null, 'getPHID');
      $children = mgroup($children, 'getParentRolePHID');
    } else {
      $children = array();
    }

    $columns = mgroup($columns, 'getRolePHID');
    foreach ($boards as $board_phid => $board) {
      ...
      if ($board->getHasMilestones()) {
        ...
        foreach ($child_roles as $child_phid => $child) {
  ...
  private function loadPositions(array $boards) {
    ...
    $positions = id(new PhabricatorRoleColumnPositionQuery())
      ->setViewer($viewer)
  ...
  private function layoutBoard(
    ...
    if ($proxy_map && $object_phids) {
      ...
      $role_phids = array_fuse($role_phids);
    } else {
      $role_phids = array();
    }
    ...
    // Build a map from every role that any task is tagged with to the
    // ancestor role which has a column on this board, if one exists.
    $ancestor_map = array();
    foreach ($roles as $phid => $role) {
      if (isset($proxy_map[$phid])) {
        ...
        foreach ($role->getAncestorRoles() as $ancestor) {
          ...
          if (isset($proxy_map[$ancestor_phid])) {
            foreach ($seen as $role_phid) {
              $ancestor_map[$role_phid] = $proxy_map[$ancestor_phid];
            }
          }
        }
      }
    ...
    foreach ($object_phids as $object_phid) {
      ...
      $proxy_hits = array();
      if ($proxy_map) {
        $object_role_phids = $edge_query->getDestinationPHIDs(
          array(
            $object_phid,
          ));

        foreach ($object_role_phids as $role_phid) {
          if (isset($ancestor_map[$role_phid])) {
            $proxy_hits[] = $ancestor_map[$role_phid];
          }
        }
      }
      ...
      if ($proxy_hits) {
        ...
        foreach ($proxy_hits as $proxy_hit) {
          $new_position = id(new PhabricatorRoleColumnPosition())
            ->setBoardPHID($board_phid)
