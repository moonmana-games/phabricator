...
final class PhabricatorRoleBoardViewController
  ...
  public function handleRequest(AphrontRequest $request) {
    ...
    $board_uri = $role->getWorkboardURI();
    ...
    if (!$columns || !$role->getHasWorkboard()) {
      ...
      if (!$has_normal_columns) {
        if (!$can_edit) {
          $content = $this->buildNoAccessContent($role);
        } else {
          $content = $this->buildInitializeContent($role);
        }
      } else {
        if (!$can_edit) {
          $content = $this->buildDisabledContent($role);
        } else {
          $content = $this->buildEnableContent($role);
        }
      }
    ...
    foreach ($columns as $column) {
      ...
      if ($proxy && !$proxy->isMilestone()) {
        // TODO: For now, don't show subrole columns because we can't
    ...
    $rendering_engine = id(new PhabricatorBoardRenderingEngine())
      ...
      ->setExcludedRolePHIDs($container_phids);
    ...
    foreach ($visible_columns as $column_phid => $column) {
      ...
      if ($column->isHidden()) {
        $panel->addClass('role-panel-hidden');
      }

      $column_menu = $this->buildColumnMenu($role, $column);
    ...
    $ordering_map = PhabricatorRoleColumnOrder::getEnabledOrders();
    ...
    $behavior_config = array(
      'moveURI' => $this->getApplicationURI('move/'.$role->getID().'/'),
    ...
    $manage_menu = $this->buildManageMenu($role, $state->getShowHidden());

    $header_link = phutil_tag(
      'a',
      array(
        'href' => $this->getApplicationURI('profile/'.$role->getID().'/'),
      ),
      $role->getName());

    $board_box = id(new PHUIBoxView())
      ->appendChild($board)
      ->addClass('role-board-wrapper');
    ...
    $background = $role->getDisplayWorkboardBackgroundColor();
  ...
  private function buildColumnMenu(
    ...
    if ($column->getProxyPHID()) {
      ...
      $default_phid = $column->getRolePHID();
    }
    ...
    $role_move_uri = urisprintf('bulkmove/%d/role/', $column->getID());
    $role_move_uri = $state->newWorkboardURI($role_move_uri);

    $column_items[] = id(new PhabricatorActionView())
      ->setIcon('fa-arrows')
      ->setName(pht('Move Tasks to Role...'))
      ->setHref($role_move_uri)
      ->setWorkflow(true);
    ...
    $edit_uri = 'board/'.$role->getID().'/edit/'.$column->getID().'/';
    $column_items[] = id(new PhabricatorActionView())
  ...
  private function buildTriggerMenu(PhabricatorRoleColumn $column) {
  ...
  private function buildInitializeContent(PhabricatorRole $role) {
    $request = $this->getRequest();
    $viewer = $this->getViewer();
    ...
    if ($set_default) {
      $this
        ...
        ->adjustDefault(PhabricatorRole::ITEM_WORKBOARD);
    }
    ...
    // TODO: Tailor this UI if the role is already a parent role. We
    // should not offer options for creating a parent role workboard, since
    ...
    $new_selector = id(new AphrontFormRadioButtonControl())
      ...
      ->addButton(
        'import',
        pht('Import Columns'),
        pht('Import board columns from another role.'));
    ...
    $default_checkbox = id(new AphrontFormCheckboxControl())
      ...
      ->addCheckbox(
        'default',
        1,
        pht('Make the workboard the default view for this role.'),
        true);
    ...
    $form = id(new AphrontFormView())
      ...
      ->appendRemarkupInstructions(
        pht('The workboard for this role has not been created yet.'))
  ...
  private function buildNoAccessContent(PhabricatorRole $role) {
    $viewer = $this->getViewer();
    ...
    return $this->newDialog()
      ...
      ->appendParagraph(
        pht(
          'The workboard for this role has not been created yet, '.
          'but you do not have permission to create it. Only users '.
          'who can edit this role can create a workboard for it.'))
      ->addCancelButton($profile_uri);
  }


  private function buildEnableContent(PhabricatorRole $role) {
    $request = $this->getRequest();
    $viewer = $this->getViewer();
  ...
  private function buildDisabledContent(PhabricatorRole $role) {
    $viewer = $this->getViewer();
    ...
    return $this->newDialog()
      ...
      ->appendParagraph(
        pht(
          ...
          'to enable it. Only users who can edit this role can restore '.
          'the workboard.'))
      ->addCancelButton($profile_uri);
  }
