...
final class PhabricatorRoleSlugsTransaction
  extends PhabricatorRoleTransactionType {

  const TRANSACTIONTYPE = 'role:slugs';
  ...
  public function getTitle() {
    ...
    if ($add && $rem) {
      return pht(
        '%s changed role hashtag(s), added %d: %s; removed %d: %s.',
        $this->renderAuthor(),
        count($add),
      ...
      return pht(
        '%s added %d role hashtag(s): %s.',
        $this->renderAuthor(),
        count($add),
        $this->renderValueList($add));
    } else if ($rem) {
        return pht(
          '%s removed %d role hashtag(s): %s.',
          $this->renderAuthor(),
          count($rem),
          $this->renderValueList($rem));
    }
  ...
  public function validateTransactions($object, array $xactions) {
    ...
    if ($new) {
      $slugs_used_already = id(new PhabricatorRoleSlug())
        ->loadAllWhere('slug IN (%Ls)', $new);
    } else {
      // The role doesn't have any extra slugs.
      $slugs_used_already = array();
    }

    $slugs_used_already = mgroup($slugs_used_already, 'getRolePHID');
    foreach ($slugs_used_already as $role_phid => $used_slugs) {
      if ($role_phid == $object->getPHID()) {
        continue;
      }
      ...
      $errors[] = $this->newInvalidError(
        pht(
          '%s role hashtag(s) are already used by other roles: %s.',
